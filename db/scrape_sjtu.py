...
T=[d for d in DB if d["sch"]=="s" and d["dept"] in URL and re.match(r"^[一-鿿]{2,4}$",d["n"])
   and (d["research"].startswith("—") or not d.get("email") or "名录未给" in d.get("title",""))]
def fetch(d):
...
STOP=r"工作经历|教育背景|教育经历|个人简介|简介|联系方式|Email|邮箱|荣誉|获奖|学术兼职|代表性|论文|项目|招生|Publications|Experience|Education"
def parse(name,body):
    ...
    if r:
        rs=re.sub(r"[•●▪·]"," ",r.group(1)); rs=re.sub(r"\s+"," ",rs).strip(" ;；,，。")
...
with ThreadPoolExecutor(8) as ex:
    for d,url,body in ex.map(fetch,T):
        ...
        ok+=1
        if p.get("research") and d["research"].startswith("—"): d["research"]=p["research"]; filled["research"]+=1
